...
const ApproveContributions = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setStats({
          ...
          approvedToday: response.data.totalApproved || 0,
          rejectedToday: 0,
          avgResponseTime: 0
        });
      }
  ...
  const handleApprove = async (contribution) => {
    try {
      ...
      if (response.success) {
        setContributions(contributions.map(c => 
          c._id === contribution._id ? { ...c, status: 'approved' } : c
        ));
        toast.success('Contribution approved successfully');
        fetchStats();
      } else {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (response.success) {
        setContributions(contributions.map(c => 
          c._id === selectedContribution._id ? { ...c, status: 'rejected' } : c
        ));
        toast.success('Contribution rejected');
        setShowRejectModal(false);
        setRejectionReason('');
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {currentItems.map((contribution) => (
                ...
                >
                  <td className="py-4 px-6">
                    <span className="font-medium text-gray-800">#{contribution._id?.slice(-6)}</span>
                  </td>
                  <td className="py-4 px-6">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center">
                      ...
                      <span className="text-gray-800">{contribution.user?.name || 'Unknown'}</span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getPlasticTypeColor(contribution.plasticType)}`}>
              ...
              <div className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">User</p>
                      <p className="font-medium text-gray-800">{selectedContribution.user?.name || 'Unknown'}</p>
                      <p className="text-sm text-gray-600">Email: {selectedContribution.user?.email || 'N/A'}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">Collection Point</p>
                      <p className="font-medium text-gray-800">{selectedContribution.collectionPoint || 'Not specified'}</p>
                    </div>
                  </div>
                  ...
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <p className="text-sm text-gray-500">Quantity</p>
                        <p className="font-medium text-gray-800">{selectedContribution.quantity} {selectedContribution.unit || 'kg'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Points Earned</p>
                        <p className="font-medium text-primary-600">{selectedContribution.pointsEarned}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Submitted Date</p>
                        <p className="font-medium text-gray-800">{format(new Date(selectedContribution.createdAt), 'PPP')}</p>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {selectedContribution.images.map((image, index) => (
                          <div key={index} className="aspect-square bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                            <img src={image.url} alt={`Contribution ${index + 1}`} className="w-full h-full object-cover" />
                          </div>
                        ))}
                      </div>
                    </div>
